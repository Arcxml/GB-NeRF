...
#from inpainting.models.attention_processor import LoRAAttnProcessor
...
def random_mask(im_shape, ratio=1, mask_full_image=False):
    ...
    print(f"{im_shape[0]= } {im_shape[1]=}")
    #size = (random.randint(0, int(im_shape[0] * ratio)), random.randint(0, int(im_shape[0] * ratio)))
    size = (random.randint(0, int(im_shape[0] )), random.randint(0, int(im_shape[1])))
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if instance_mask_root is not None:
            ...
            self.instance_masks_path = list(Path(instance_mask_root).iterdir())
            self.num_instance_masks = len(self.instance_masks_path)
        else:
            self.instance_mask_root = None
    ...
    def __getitem__(self, index):
        example = {}
        # instance_image = np.load(self.instance_images_path[index % self.num_instance_images])
        # instance_image = Image.fromarray(np.uint8(instance_image))
        ...
        if image_path.suffix.lower() == '.npy':
            instance_image = np.load(image_path)
            instance_image = Image.fromarray(np.uint8(instance_image))
        elif image_path.suffix.lower() in ['.jpg', '.jpeg', '.png']:
            instance_image = Image.open(image_path)
        else:
            raise ValueError(f"Unsupported file format: {image_path.suffix}")

        # 从captions文件夹中找到对应的txt文件
        caption_path = Path("/gdata2/zhangml/projects/MVIP-NeRF-main/data/captions") / f"{base_name}.txt"
        if caption_path.exists():
            with open(caption_path, 'r') as f:
                instance_prompt = f.read().strip()
        else:
            raise ValueError(f"Caption file not found for image: {image_path}")
        ...
        if self.instance_mask_root:
            ...
            example["instance_PIL_masks"] = instance_mask
        else:
            example["instance_masks"] = None
            example["instance_PIL_masks"] = None
...
def main():
    ...
    def collate_fn(examples):
        ...
        if args.instance_mask_dir:
            batch = {"input_ids": input_ids, "pixel_values": pixel_values, "masks": masks, "masked_images": masked_images, "instance_masks": instance_mask}
        else :
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if "instance_masks" in batch and batch["instance_masks"] is not None:
                    instance_masks = batch["instance_masks"].to(device=latents.device, dtype=weight_dtype)
                    if instance_masks is not None:
                        instance_masks = torch.nn.functional.interpolate(
                        instance_masks, size=(args.resolution // 8, args.resolution // 8))
                    instance_masks = instance_masks.reshape(-1, 1, args.resolution // 8, args.resolution // 8)
                ...
                if args.with_prior_preservation:
                    ...
                    # Compute instance loss
                        loss = ((1 - instance_masks) * F.mse_loss(noise_pred.float(), target.float(), reduction="none")).mean([1, 2, 3]).mean()
                    else:
                    ...
                    # noise_pred.shape = (N, 3, H, W)
                    # mask.shape = (N, 1, H, W)
                    # loss = (mask * F.mse_loss(noise_pred.float(), target.float(), reduction="none")).mean()
