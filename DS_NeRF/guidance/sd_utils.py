...
class StableDiffusion(nn.Module):
    def __init__(self, device, clipaway_config, fp16, vram_O, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98], model_path=None,use_clipaway=False):
        super().__init__()
        ...
        #     safety_checker = None
        # )
        # model_path = "checkpoint/checkpoint-500"

        # self.pipe.unet.load_attn_procs(model_path)
        if self.model_path:
            print(f"[INFO] loading lora weights from {self.model_path}")
...
#         run.pipe = Pipeline.from_pretrained(
...
# )
    ...
    def train_step_sd_normal(self, i, mask, prompt, pred_normal_map, guidance_scale=100, normal_start = 0, as_latent=False, grad_scale=1,
                   save_guidance_path:Path=None, neg_prompt=None, use_csd=False, w1=1, w2=0.5,w3=0.5,use_negative=0):
        ...
        print(f"{pred_normal_map.shape= } ")
        pred_normal_map = pred_normal_map * 2 - 1
        print(f"{pred_normal_map.max()=}  {pred_normal_map.min()=}")
        height = latent_size
        width = latent_size
        callback_steps = 1
        ...
        if use_csd:
                 # 1. Encode input prompt, use csd
            prompt_embeds = self.pipe._encode_prompt_csd(
            prompt,
            self.device,
        ...
        # 5. Unet
        with torch.no_grad(): 
            if do_classifier_free_guidance and use_csd:
            ...
            # perform guidance
            if do_classifier_free_guidance and use_csd:
                noise_pred_null, noise_pred_uncond, noise_pred_text = noise_pred.chunk(3)
                noise_pred = w1 * noise_pred_text + w3 * noise_pred_null - w2 * noise_pred_uncond
            elif do_classifier_free_guidance:
            ...
            if use_csd:
                ...
                grad = w * noise_pred - noise #* mask  sds
    ...
    def train_step_sd(self, i, mask, prompt, pred_rgb, guidance_scale=100, as_latent=False, grad_scale=1,
        ...
        print(f"{pred_rgb.max()=}  {pred_rgb.min()=}")
        height = latent_size
        width = latent_size
        callback_steps = 1
        num_images_per_prompt = 1
    ...
    def train_step_colla_sds(self, i, mask_nn, prompt, pred_rgb_nn, guidance_scale=100, as_latent=False, grad_scale=1,
                   save_guidance_path:Path=None, neg_prompt=None, use_csd=False, w1=1, w2=0.5,use_negative=0    ):
        ...
        for i in range(NN):
            ...
            with torch.no_grad():    
                ...
                if do_classifier_free_guidance and use_csd:
                    noise_pred_null, noise_pred_uncond, noise_pred_text = noise_pred.chunk(3)
                    noise_pred = w1 * noise_pred_text + (w2 - w1) * noise_pred_null - w2 * noise_pred_uncond
                elif do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
